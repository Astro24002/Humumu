...
- **Web 可分享筛选 URL**：公开广场（`/`：`content_type`/`source_type`/`journal_id`/`page`）、期刊广场（`/journals`：`q`/`content_type`/`source_type`/`sort`/CAS `major|minor|zone|top`/`page`）、我的更新（`/my?filter=`）、通知（`/my/notifications`：`status`/`channel`）、期刊详情论文分页（`/journals/:id?page=`），以及管理端 journals/users/requests 列表筛选与 `page`、CAS 分类 `year`，均通过 `router.replace` 同步 query，支持深链与刷新保持。
